...
from imitation_modelling.schemas import TaskRunStatusLog, SystemTime, TaskRunStatus, TaskBatchProviderType
...
class TacticalPIDController:
    ...
    def compute(self, current_utilization: float, dt: float) -> Tuple[int, Dict]:
        ...
        print(f"{self.Bbase=}, {u=}, {batch_size=}")
        ...
        return batch_size, {
            ...
            'saturated': saturated,
            'saturation_type': 'max' if batch_size == self.Bmax else 'min' if batch_size == self.Bmin else None  # TODO
        }
...
class StrategicBoundaryAdapter:
    ...
    def _adapt_boundaries(self):
        ...
        print("is_stable, max_saturation_count, window, error_mean", is_stable, max_saturation_count, window, error_mean)
        if (is_stable and
                max_saturation_count > window * 0.7 and
                error_mean < self.error_rate_threshold):
            new_Bmax = int(current_Bmax * 3)
...
class PIDTaskBatchProvider(TaskBatchProvider):
    """
    Главный контроллер с двухуровневым управлением
    """
    type: TaskBatchProviderType = TaskBatchProviderType.PID


    def __init__(self, broker: Broker, task_run_status_repo: TaskRunStatusRepo,
                 task_run_metric_provider: TaskRunMetricProvider, system_time: SystemTime, metric_collector: MetricCollector,
                 target_utilization: float = 0.75, pid_params: Optional[Dict] = None,
                 strategic_params: Optional[Dict] = None, **kwargs):
    ...
    def _check_stability(self, window: int = 10) -> bool:
        ...
        print(f"{recent_quality=}")
        mean_quality = float(np.mean(recent_quality))
        std_quality = float(np.std(recent_quality))
    ...
    def need_dump(self):
        ...
        if time.perf_counter() - self.start_time >= 170:
            return True
        return False
    ...
    def iter(self) -> Iterator[TaskRunStatusLog]:
        last_metrics = self.metric_collector.last_metrics
        system_metrics = SystemMetrics(timestamp=last_metrics['time'],
                                       success_count=last_metrics['completed'],
                                       error_rate=last_metrics['returnFrequency'],
                                       throughput=last_metrics['succeedFrequency'],
                                       avg_latency=last_metrics['queuedAvgDuration'] + last_metrics['executionAvgDuration'],
                                       queue_depth=last_metrics['queuedCount'],
                                       queue_capacity=last_metrics['total'],
                                       error_count=int(last_metrics['returnFrequency'] * self.metric_collector.metric_provider.metric_period.seconds))
        next_batch_size, _ = self.get_next_batch_size(system_metrics)
        tasks_count = 0
        for actual_task_run_status_log in self._task_run_status_repo.iter_actual_statuses({TaskRunStatus.WAITING}):
            ...
            tasks_count += 1
            if tasks_count >= next_batch_size:
                break
        if self.need_dump():
            self.export_history("pid_history.json")
